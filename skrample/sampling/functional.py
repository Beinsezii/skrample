...
class FastHeun(FunctionalAdaptive, FunctionalSinglestep, FunctionalHigher):
    ...
    def step[T: Sample](
        ...
        eta_t = model_transform.to_eta(sigma_from, self.schedule.sigma_transform)
        eta_s = model_transform.to_eta(sigma_to, self.schedule.sigma_transform)
        dt = eta_s - eta_t
        ...
        while step <= indices[-1]:
            ...
            if step_next < len(schedule):
                sample_high, sample_low = step_tableau(
                    tab, sample, model, model_transform, step, schedule, self.schedule.sigma_transform, step_size
                )

                eta_t = model_transform.to_eta(schedule[step][1], self.schedule.sigma_transform)
                eta_s = model_transform.to_eta(schedule[step_next][1], self.schedule.sigma_transform)
                delta = abs(eta_s - eta_t)

                eta_s1 = model_transform.to_eta(
                    schedule[step_next + step_size][1] if step_next + step_size < len(schedule) else 0,
                    self.schedule.sigma_transform,
                )
                delta_next = abs(eta_s1 - eta_s)
                ...
                sample_high = step_tableau(
                    tab[:2], sample, model, model_transform, step, schedule, self.schedule.sigma_transform, step_size
                )[0]
