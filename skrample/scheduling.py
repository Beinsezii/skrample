...
class ScheduleCommon(SkrampleSchedule):
    ...
    If negative, the timestep scale is flipped such that during inference
    we move from T=0 -> T=N rather than T=N -> T=0, so T=N represents the clean result."""

    @functools.cached_property
    def all_points(self) -> NPPoints:
        "Returns all points over `base_timesteps` with LRU cache"
        num = abs(self.base_timesteps)
        if num <= 1:  # typically 0..=1 represents a floating point schedule with infinite interpolation
            num = 10_000
        return self.points_np(np.linspace(0, 1, num))
...
class Scaled(ScheduleCommon):
    ...
    def _points(self, t: NPSequence) -> NPPoints:
        ...
        return np.stack(
            [
                ((1 - t) if self.base_timesteps < 0 else t) * abs(self.base_timesteps),
                *self.space.normalize(sigmas),
            ],
            1,
        )
...
class Linear(ScheduleCommon):
    ...
    def _points(self, t: NPSequence) -> NPPoints:
        return np.stack(
            [
                ((1 - t) if self.base_timesteps < 0 else t) * abs(self.base_timesteps),
                *self.space.normalize(t * self.sigma_start),
            ],
            axis=1,
        )
    ...
    def _sigmas_to_points(self, sigmas: NPSequence, alphas: NPSequence) -> NPPoints:
        return np.stack(
            [
                ((self.sigma_start - sigmas) if self.base_timesteps < 0 else sigmas)
                * (abs(self.base_timesteps) / self.sigma_start),
                sigmas,
                alphas,
            ],
            axis=1,
