...
from skrample.scheduling import FixedSchedule, FlowMatching, SigmaSpace, VariancePreserving
...
def dual_sample(
    ...
    if isinstance(b, FlowMatchEulerDiscreteScheduler):
        b_sample = b.scale_noise(sample=b_sample, timestep=b.timesteps[steps.start].unsqueeze(0), noise=initial_noise)
    else:
        b_sample = b.add_noise(
            original_samples=b_sample, noise=initial_noise, timesteps=b.timesteps[steps.start].unsqueeze(0)
        )

    sigma_space = FlowMatching() if isinstance(model_transform, FlowModel) else VariancePreserving()
    ...
    for step in steps:
        ...
        a_step = Step.from_int(step, steps.stop)
        a_output = fake_model(a.scale_input(a_sample, skrample_schedule.ipoint(a_step.time_from)))
        sampled = a.sample(
            a_sample,
            a_output,
            a_step,
        ...
        if isinstance(b, FlowMatchEulerDiscreteScheduler):
            ...
            b_output = fake_model(b.scale_model_input(sample=b_sample, timestep=b.timesteps[step]))

        if "generator" in signature(b.step).parameters:  # why, diffusers, why
            b_sample = b.step(model_output=b_output, sample=b_sample, timestep=b.timesteps[step], generator=seed)[0]
        else:
            b_sample = b.step(model_output=b_output, sample=b_sample, timestep=b.timesteps[step])[0]
...
def test_dpm(predictor: tuple[DiffusionModel, str], order: int, stochastic: bool, steps: range) -> None:
    ...
    compare_tensors(
        *dual_sample(
        ...
        margin=1e-5 if order >= 3 else 1e-8,
    )
...
@pytest.mark.parametrize(
    ("model_transform", "derivative_transform", "sigma_space", "diffusers_scheduler", "steps"),
    (
        (mt, dt, st, ds, s)
...
def test_heun(
    ...
    skrample_schedule = FixedSchedule.from_regular(
        diffusers_scheduler.timesteps,
        diffusers_scheduler.sigmas[:-1],
        sigma_space,
    )
